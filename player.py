...
class Karen(pygame.sprite.Sprite):
	...
	def __init__(self):
		...
		self.speed = 3
		self.speed_bonus = 5
		self.img = pygame.image.load(dados.KAREN)
		self.rect = pygame.Rect(self.x, self.y, 25, 43)
		self.img_speed_init = 14
		self.img_speed = self.img_speed_init
		self.img_pos = 0
		self.wins_the_game = pygame.image.load(dados.KAREN_WINS)
		self.flag_winner = pygame.image.load(dados.KAREN_FLAG)
		self.frozen = pygame.image.load(dados.FROZEN_KAREN)
		self.sit_chair = pygame.image.load(dados.KAREN_SIT)
		self.life = [pygame.image.load(dados.K_3LIFES), pygame.image.load(dados.K_2LIFES), pygame.image.load(dados.K_1LIFE)]
		...
		self.dancing = [pygame.image.load(dados.KAREN_DANCING1), pygame.image.load(dados.KAREN_DANCING2), 
						 pygame.image.load(dados.KAREN_DANCING3)]
		self.walking_left = [pygame.image.load(dados.KAREN_LEFT),pygame.image.load(dados.KAREN_LEFT), 
								 pygame.image.load(dados.KAREN_LEFT1)]
		self.walking_right = [pygame.image.load(dados.KAREN_RIGHT), pygame.image.load(dados.KAREN_RIGHT1), 
								pygame.image.load(dados.KAREN_RIGHT2)]
		self.walking_up = [pygame.image.load(dados.KAREN_BACK), pygame.image.load(dados.KAREN_UP1), 
							 pygame.image.load(dados.KAREN_UP2)]
		self.walking_down = [pygame.image.load(dados.KAREN), pygame.image.load(dados.KAREN_DOWN1),
							  pygame.image.load(dados.KAREN_DOWN2)]
		self.img_max = len(self.dancing) -1
		self.game_over_sound = pygame.mixer.Sound(dados.GAME_OVER)
	...
	def update(self, music_playing, frozen, sit):
		...
		if frozen:
			...
			if keys_pressed[K_LEFT] and self.x >= -1:
				if not music_playing:
					...
					if self.img_speed <= 0:
						...
						if self.img_pos == self.img_max:
							...
							self.img_pos += 1
				self.x -= self.speed
			elif keys_pressed[K_RIGHT] and self.x <= 780:
				if not music_playing:
					self.img_speed -= 4
					if self.img_speed <= 0:
						...
						if self.img_pos == self.img_max:
							...
							self.img_pos += 1
				self.x += self.speed
			elif keys_pressed[K_UP] and self.y >= 64:
				if not music_playing:
					self.img_speed -= 4
					if self.img_speed <= 0:
						...
						if self.img_pos == self.img_max:
							...
							self.img_pos += 1
				self.y -= self.speed
			elif keys_pressed[K_DOWN] and self.y <= 620:
				if not music_playing:
					self.img_speed -= 4
					if self.img_speed <= 0:
			...
			if music_playing:
				...
				if self.img_speed <= 0:
					...
					if self.img_pos == self.img_max:
						...
						self.img_pos += 1
			self.rect = pygame.Rect(self.x, self.y, 29, 43)
	...
	def draw(self, display, damage, wins, sit):
		...
		if damage == 3:
			...
			display.blit(self.life[damage], (30,0))
		if wins > 0:
			display.blit(self.wins[wins-1], (150,5))

	def collide_detect_karen(self, wall):
